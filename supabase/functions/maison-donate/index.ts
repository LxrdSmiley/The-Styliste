...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const idempotencyKey = typeof body.idempotency_key === "string"
      ...
      : "";
    if (amount <= 0 || !idempotencyKey) return json({ error: "INVALID_PAYLOAD" }, 400);
    ...
    const { data, error } = await admin.rpc("edge_maison_donate_atomic", {
      p_player_id: user.id,
      p_amount: amount,
    ...
    console.error("maison-donate", correlationId, error);
    const message = error instanceof Error ? error.message : "";
    if (message.includes("INSUFFICIENT_CAPITAL")) return json({ error: "INSUFFICIENT_CAPITAL" }, 409);
    if (message.includes("NOT_A_MEMBER")) return json({ error: "NOT_A_MEMBER" }, 403);
    return json({ error: "DONATION_FAILED", correlation_id: correlationId }, 500);
  }
});
