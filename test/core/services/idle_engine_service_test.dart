...
void main() {
  ...
  test('IdleEngineService uses process_idle_income through rpc', () {
    final String source =
        File('lib/core/services/idle_engine_service.dart').readAsStringSync();

    expect(source, contains('SupabaseService.client.rpc'));
    expect(source, contains('SupabaseConstants.fnProcessIdleIncome'));
    expect(source, contains("'p_player_id': playerId"));
    expect(source, isNot(contains('invokeFunction(')));
    expect(SupabaseConstants.fnProcessIdleIncome, 'process_idle_income');
  });
}
