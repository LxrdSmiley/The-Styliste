...
class IdleIncomeResult {
  ...
  double get fillPercent => 
    warehouseCapacity > 0 
      ? (inventoryValue / warehouseCapacity) * 100 
      : 0.0;
  ...
  Future<void> _invokeAndNotify() async {
    try {
      ...
      final IdleIncomeResult result = IdleIncomeResult(
        addedToInventory: (response['added_to_inventory'] as num?)?.toDouble() ?? 0.0,
        inventoryValue: (response['new_inventory'] as num?)?.toDouble() ?? 0.0,
        warehouseCapacity: (response['warehouse_capacity'] as num?)?.toDouble() ?? 5000.0,
        isWarehouseFull: response['is_full'] as bool? ?? false,
        secondsElapsed: (response['seconds_elapsed'] as num?)?.toDouble() ?? 0.0,
        idleRevenuePerHour: (response['idle_revenue_per_hour'] as num?)?.toDouble() ?? 0.0,
      );
