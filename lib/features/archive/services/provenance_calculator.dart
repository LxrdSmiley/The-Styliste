...
class ProvenanceCalculator {
  ...
  static double calculateMultiplier(int transferCount, bool hasSovereignProvenance) {
    // Base multiplier
    double multiplier = 1.0;
    
    // Transfer bonus (capped at 10 owners = +100%)
    final double transferBonus = (math.min(transferCount, _maxTransferCount) * _perOwnerBonus)
  ...
  static int calculateEffectiveHype(int baseHype, int transferCount, bool hasSovereignProvenance) {
    final double multiplier = calculateMultiplier(transferCount, hasSovereignProvenance);
    return (baseHype * multiplier).round();
  }
  
  /// Get formatted provenance description for UI
  static String getProvenanceDescription(int transferCount, bool hasSovereignProvenance) {
    ...
    if (transferCount > 0) {
      ...
      if (transferCount > _maxTransferCount) {
        ...
        parts.add('$transferCount previous ${transferCount == 1 ? 'owner' : 'owners'}');
      }
    }
  ...
  static ProvenanceBreakdown getBreakdown(int transferCount, bool hasSovereignProvenance) {
    final double transferBonus = (math.min(transferCount, _maxTransferCount) * _perOwnerBonus)
        .clamp(0.0, _maxTransferBonus);
...
class ProvenanceBreakdown {
  ...
  String get formattedTotal => ProvenanceCalculator.formatMultiplier(totalMultiplier);
}
